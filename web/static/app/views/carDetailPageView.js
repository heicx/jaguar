	...
	var carDetailView = Backbone.View.extend({
		...
		shareToWechat: function (evt) {
			...
			if (evt.type !== "mouseenter") {
				...
				$qr.find("img").attr("src", "/usedcar/qrcode?url=" + "http://beta.m.jaguar.iautos.cn/usedcar/" + carId + ".html").parent().show();
			}
		},
		...
		pageScroll: function () {
			...
			$(window).scroll(function (event) {
				...
				if(listFixedHeight > 0 && Backbone.history.fragment.indexOf("detail") > -1) {
		...
		displayCarouselImgs: function (evt) {
			...
			if (evt.type !== "mouseenter") {
				$(".carousel-list").fadeOut();
			} else {
				$(".carousel-list").fadeIn();
			}
		}
	});
